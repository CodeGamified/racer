...
namespace Racer.UI
{
    ...
    /// </summary>
    public class RacerStatusPanel : TerminalWindow
    {
        ...
        public void Bind(RacerMatchManager match, RacerProgram playerProgram)
        {
            _match = match;
        ...
        protected override void Render()
        {
            ...
            RenderSection(SEC_RACE,     BuildRaceSection);
        ...
        private string[] BuildRaceSection(int maxRows)
        {
            var lines = new List<string>();

            Color32 accent = TUIGradient.Sample(0.3f);
            lines.Add($" {TUIColors.Fg(accent, TUIGlyphs.DiamondFilled)} {TUIColors.Bold("RACE")}");
            ...
            if (_match != null)
            {
                ...
                // Lap / total
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightCyan, $"LAP {_match.CurrentLap}/{_match.TotalLaps}")}");

                // Times
                string raceTime = FormatTime(_match.RaceTime);
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightGreen, raceTime)} {emdash} RACE TIME");

                string lapTime = FormatTime(_match.CurrentLapTime);
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightYellow, lapTime)} {emdash} LAP TIME");

                string bestLap = _match.BestLapTime < float.MaxValue
                    ? FormatTime(_match.BestLapTime) : "--:--.--";
                lines.Add($"  {bestLap} {emdash} BEST LAP");

                string bestRace = _match.BestRaceTime < float.MaxValue
                    ? FormatTime(_match.BestRaceTime) : "--:--.--";
                lines.Add($"  {bestRace} {emdash} BEST RACE");

                // Progress
                int pctInt = Mathf.RoundToInt(_match.Progress * 100f);
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightMagenta, $"{pctInt}%")} {emdash} PROGRESS");

                // Waypoints
                lines.Add($"  {_match.WaypointsHit} {emdash} WAYPOINTS");

                lines.Add($"  {_match.RacesCompleted} {emdash} RACES");

                if (_match.RaceFinished)
                    lines.Add($"  {TUIColors.Fg(TUIColors.BrightGreen, "🏁 FINISHED")}");
            }
            else
            {
        ...
        {
            if (seconds >= float.MaxValue * 0.5f) return "--:--.--";
